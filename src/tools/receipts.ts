  ...
  server.registerTool(
    'sw_get_receipt',
    {
      ...
      inputSchema: z.object({
        ...
        size: z
          .enum(['original', 'large'])
          .describe(
            "Which stored rendition to fetch. Defaults to 'original' (full quality, and the only one a PDF receipt has). Falls back to the other rendition when the requested one is absent.",
          )
          .optional(),
        inline: z
          .boolean()
          .describe(
            `Return the receipt bytes in the result — an image block for images, an embedded resource for PDFs and everything else. Only for receipts under ${MAX_INLINE_BYTES} bytes.`,
          )
          .optional(),
        extract_text: z
          .boolean()
          .describe(
            "Also return the PDF's text layer as `text`. Ideal for looking up line items or totals without transferring the file. PDFs only; a scanned receipt has no text layer to extract.",
          )
          .optional(),
        output_dir: z
          .string()
    ...
    async ({ id, size, inline, extract_text, output_dir, write }) => {
      ...
      const expense = await client.request<{
        expense?: { receipt?: ExpenseReceipt | null } | null;
      }>('GET', `/get_expense/${id}`);
      ...
      if (path === undefined && !inlined && text === undefined) {
        ...
        if (extract_text !== true && mimeType === PDF_MIME)
          routes.push("extract_text:true for the PDF's text");
        const advice =
          routes.length > 0
      ...
      const result = minifiedResult({
        ...
        ...(served !== wanted
          ? {
              ...
              note: `Expense ${id} has no ${wanted} rendition; fetched ${served}.`,
            }
          : {}),
        ...(path !== undefined ? { path } : {}),
